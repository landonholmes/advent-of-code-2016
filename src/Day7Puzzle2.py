instructions = """rhamaeovmbheijj[hkwbkqzlcscwjkyjulk]ajsxfuemamuqcjccbc
...
def check_if_aba(some_string):
    if some_string[0] == some_string[2] and some_string[0] != some_string[1]:
        return True
    else:
        return False


def check_if_aba_are_reversed(string_a, string_b):
    if string_a[0] == string_b[1] and string_a[2] == string_b[1] and string_a[1] == string_b[0] and string_a[1] == string_b[2]:
        return True
    else:
        return False


for ip_address in instructions.split("\n"):
    hypernet_sequence = True  # start on true because we begin loop with toggle, so it will actually start with false
    regular_aba = []
    hypernet_sequence_aba = []

    for ip_address_piece in re.findall(r"[\w']+", ip_address):
        hypernet_sequence = not hypernet_sequence
        current_ip_index = 0
        while current_ip_index+3 <= len(ip_address_piece):
            current_string_to_check = ip_address_piece[current_ip_index:current_ip_index+3]

            if check_if_aba(current_string_to_check):
                if hypernet_sequence:
                    hypernet_sequence_aba.append(current_string_to_check)
                else:
                    regular_aba.append(current_string_to_check)

            current_ip_index += 1

    if len(regular_aba) > 0 and len(hypernet_sequence_aba) > 0:
        success = False
        for single_regular_aba in regular_aba:
            for single_hypernet_aba in hypernet_sequence_aba:
                if check_if_aba_are_reversed(single_regular_aba, single_hypernet_aba):
                    success = True

        if success:
            ssl_ip.append(ip_address)
