...
def check_if_abba(some_string):
    if some_string[0] == some_string[3] and some_string[1] == some_string[2] and some_string[0] != some_string[1]:
        return True
    else:
        return False
...
for ip_address in instructions.split("\n"):
    ...
    regular_abba = ""
    hypernet_sequence_abba = ""

    for ip_address_piece in re.findall(r"[\w']+", ip_address):
        hypernet_sequence = not hypernet_sequence
        current_ip_index = 0
        while current_ip_index+4 <= len(ip_address_piece):
            current_string_to_check = ip_address_piece[current_ip_index:current_ip_index+4]

            if check_if_abba(current_string_to_check):
                if hypernet_sequence:
                    hypernet_sequence_abba = current_string_to_check
                else:
                    regular_abba = current_string_to_check

            current_ip_index += 1

    if len(regular_abba) > 0 and len(hypernet_sequence_abba) == 0:
        tls_ip.append(ip_address)
